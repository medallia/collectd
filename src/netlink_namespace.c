...
static int link_filter_cb(const struct nlmsghdr *nlh,
  ...
  mnl_attr_for_each(attr, nlh, sizeof(*ifm)) {
    ...
    if (mnl_attr_validate(attr, MNL_TYPE_STRING) < 0) {
      ERROR("netlink_namespace plugin: link_filter_cb: IFLA_IFNAME mnl_attr_validate "
            "failed.");
      return MNL_CB_ERROR;
    }
  ...
  mnl_attr_for_each(attr, nlh, sizeof(*ifm)) {
    ...
    if (mnl_attr_validate2(attr, MNL_TYPE_UNSPEC, sizeof(*stats.stats64)) < 0) {
      ERROR("netlink_namespace plugin: link_filter_cb: IFLA_STATS64 mnl_attr_validate2 "
            "failed.");
      return MNL_CB_ERROR;
    }
  ...
  mnl_attr_for_each(attr, nlh, sizeof(*ifm)) {
    ...
    if (mnl_attr_validate2(attr, MNL_TYPE_UNSPEC, sizeof(*stats.stats32)) < 0) {
      ERROR("netlink_namespace plugin: link_filter_cb: IFLA_STATS mnl_attr_validate2 "
            "failed.");
      return MNL_CB_ERROR;
    }
...
static int qos_attr_cb(const struct nlattr *attr, void *data) {
  ...
  if (mnl_attr_get_type(attr) == TCA_STATS_BASIC) {
    if (mnl_attr_validate2(attr, MNL_TYPE_UNSPEC, sizeof(*q_stats->bs)) < 0) {
      ERROR("netlink_namespace plugin: qos_attr_cb: TCA_STATS_BASIC mnl_attr_validate2 "
            "failed.");
      return MNL_CB_ERROR;
    }
  ...
  if (mnl_attr_get_type(attr) == TCA_STATS_QUEUE) {
    if (mnl_attr_validate2(attr, MNL_TYPE_UNSPEC, sizeof(*q_stats->qs)) < 0) {
      ERROR("netlink_namespace plugin: qos_attr_cb: TCA_STATS_QUEUE mnl_attr_validate2 "
            "failed.");
      return MNL_CB_ERROR;
    }
...
static int qos_filter_cb(const struct nlmsghdr *nlh, void *args) {
  ...
  if (dev == NULL) {
    ERROR("netlink_namespace plugin: qos_filter_cb: iflist[%i] == NULL", tm->tcm_ifindex);
    return MNL_CB_ERROR;
  }
  ...
  mnl_attr_for_each(attr, nlh, sizeof(*tm)) {
    ...
    if (mnl_attr_validate (attr, MNL_TYPE_STRING) < 0) {
      ERROR(
          "netlink_namespace plugin: qos_filter_cb: TCA_KIND mnl_attr_validate failed.");
      return MNL_CB_ERROR;
    }
  ...
  DEBUG("netlink_namespace plugin: qos_filter_cb: got %s for %s (%i).", tc_type, dev,
        tm->tcm_ifindex);
  ...
  mnl_attr_for_each(attr, nlh, sizeof(*tm)) {
    ...
    if (mnl_attr_validate(attr, MNL_TYPE_NESTED) < 0) {
      ERROR("netlink_namespace plugin: qos_filter_cb: TCA_STATS2 mnl_attr_validate "
            "failed.");
      return MNL_CB_ERROR;
    }
  ...
  mnl_attr_for_each(attr, nlh, sizeof(*tm)) {
    ...
    if (mnl_attr_validate2(attr, MNL_TYPE_UNSPEC, sizeof(*ts)) < 0) {
      ERROR("netlink_namespace plugin: qos_filter_cb: TCA_STATS mnl_attr_validate2 "
            "failed.");
      return MNL_CB_ERROR;
    }
...
static int ir_init (void)
{
  ...
  if ((close(selfns) != 0) || (close(targetns) != 0)) {
    INFO("netlink_namespace plugin: ir_init: Unable to close file descritpros for selfns or targetns ");
  }
